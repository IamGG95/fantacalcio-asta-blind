...
export default function App() {
  ...
  const [serverOffsetMs, setServerOffsetMs] = useState(0); // differenza oraria stimata
  ...
  useEffect(() => {
    ...
    s.on('time:pong', ({ serverNow, echo }) => {
      ...
      if (typeof serverNow === 'number') {
        if (typeof echo === 'number') {
          const rtt = t1 - echo; const oneWay = rtt / 2; const estClientAtSend = echo + oneWay;
    ...
    s.on('connect', () => { setConnected(true); setCurrentSocketId(s.id); doTimeSync(); });
    ...
    s.on('admin:update', ({ adminId }) => { setAdminId(adminId || null); setIsAdmin(adminId && socketRef.current && socketRef.current.id === adminId); });
    ...
    s.on('auction:start', (a) => {
      ...
      setOffers([]); setWinner(null); setBidderIds(new Set()); setLastResultPlayer('');
    ...
    s.on('auction:sync', ({ serverNow, endsAt }) => {
      ...
      setTimeLeft(left);
      if (left === 0) setAuction(prev => prev ? { ...prev, endsAt: Number(endsAt) } : prev);
    });

    s.on('auction:bid:ack', ({ amount }) => { showToast(`Offerta inviata: ${amount} crediti`); });
    s.on('auction:bid:mark', ({ socketId }) => { if (socketId) setBidderIds(prev => { const next = new Set(prev); next.add(String(socketId)); return next; }); });
    ...
    s.on('auction:end', (payload) => {
      try {
        ...
        setOffers(sanitizedOffers); setWinner(sanitizedWinner); setLastResultPlayer(resultPlayer);
        setAuction(null); setTimeLeft(0); setBidderIds(new Set());
    ...
    return () => { clearInterval(syncInt); s.off(); s.disconnect(); };
  }, []);
  ...
  function joinAsParticipant() {
    ...
    setJoined(true); setIsAdmin(false);
  }
